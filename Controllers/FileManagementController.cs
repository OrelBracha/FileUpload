            ...
            var supportedFiles = new List<Record>
    {
        ...
        public async Task<IActionResult> CreateProfile([FromForm] Profile profile)
        {
            if (ModelState.IsValid)
            {
                ...
                using var fileStream = tempProfile.Picture.OpenReadStream();
                long length = fileStream.Length;
                byte[] bytes = new byte[length];
                fileStream.Read(bytes, 0, (int)tempProfile.Picture.Length);
                string myString = System.Text.Encoding.ASCII.GetString(bytes).Trim();
                listContent.Add(myString);
                return Ok();
            }
        ...
        public void readFromFile (String fileContent)
        {
            ...
            for(int i=0; i<listContent.Capacity;i++)
            {
                String tempContent = listContent[i];
